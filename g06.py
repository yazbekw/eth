...
BINANCE_CONFIG = {
    ...
    'base_url': 'https://testnet.binance.vision/api/'  # رابط Demo
}
...
class AdvancedCryptoBot:
    def __init__(self, trade_config, indicator_config, signal_config, binance_config):
        ...
        # فحص الاتصال أولاً
        self.check_demo_connection()
        
        # جلب الرصيد
        self.initial_balance = self.get_demo_balance()
    ...
    def check_demo_connection(self):
        """فحص الاتصال بـ Binance Demo"""
        try:
            # فحص وقت السيرفر أولاً
            server_time = self.make_request('time')
            if server_time:
                if ENABLE_LOGGING:
                    logger.info("✅ الاتصال بـ Binance Demo يعمل")
                return True
            else:
                if ENABLE_LOGGING:
                    logger.error("❌ لا يمكن الاتصال بـ Binance Demo")
                return False
    ...
    def make_request(self, endpoint, params=None, method='GET', signed=False):
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                if ENABLE_LOGGING:
                    logger.error(f"❌ خطأ من API: {error_msg} (كود: {response.status_code})")
                
                # إذا كان الخطأ 401، تحقق من API Keys
                if response.status_code == 401:
                    logger.error("❌ تأكد من صحة API Keys للـ Demo")
                return None
            ...
            if ENABLE_LOGGING:
                logger.error(f"❌ خطأ في الاتصال بـ Binance Demo: {e}")
            return None
        except Exception as e:
            if ENABLE_LOGGING:
    ...
    def get_demo_balance(self):
        """جلب الرصيد من Binance Demo"""
        try:
            account_info = self.make_request('account', signed=True)
            if not account_info:
                if ENABLE_LOGGING:
                    logger.warning("⚠️ استخدام الرصيد الافتراضي بسبب مشكلة اتصال")
            ...
            if usdt_balance > 0:
                if ENABLE_LOGGING:
                    logger.info(f"💰 الرصيد الفعلي من Demo: {usdt_balance} USDT")
                return usdt_balance
            else:
                if ENABLE_LOGGING:
                    logger.warning("⚠️ رصيد USDT صفر في Demo، استخدام الرصيد الافتراضي")
                return self.trade_config['initial_balance']
                
        except Exception as e:
            if ENABLE_LOGGING:
                logger.error(f"❌ خطأ في جلب الرصيد من Demo: {e}")
            return self.trade_config['initial_balance']

    def get_current_demo_balance(self):
        """جلب الرصيد الحالي من Demo"""
        try:
            account_info = self.make_request('account', signed=True)
            if not account_info:
    ...
    def update_balance_from_demo(self):
        """تحديث الرصيد المحلي من Demo"""
        new_balance = self.get_current_demo_balance()
...
"""
    ...
    def open_position(self, symbol, direction, signal_strength, row, details):
        ...
        try:
            ...
            if ENABLE_LOGGING:
                logger.info(f"🟢 فتح {direction} على {symbol} | قوة: {signal_strength:.1f}/10")
    ...
    def run_live_signal_check(self):
        ...
        try:
            ...
            for symbol in self.trade_config['symbols']:
                if ENABLE_LOGGING:
                    logger.info(f"🔍 فحص الإشارات لـ {symbol}")
...
def run_live_bot():
    ...
    schedule.every(10).minutes.do(bot.update_balance_from_demo)
