...
# ====================== الكلاس الرئيسي المطور مع بينانس Testnet ======================
class AdvancedCryptoBot:
    def __init__(self, trade_config, indicator_config, signal_config, binance_config):
        ...
        # تهيئة عميل بينانس Testnet أولاً
        self.binance_client = None
        self.initialize_binance_client()
        
        # جلب الرصيد الفعلي من حساب الديمو
        ...
        if ENABLE_LOGGING:
            logger.info(f"💰 الرصيد المبدئي من الديمو: ${self.initial_balance:.2f}")

    def initialize_binance_client(self):
        """تهيئة الاتصال ببينانس Testnet"""
        try:
            if self.binance_config['api_key'] and self.binance_config['api_secret']:
                ...
                if ENABLE_LOGGING:
                    logger.info("✅ تم الاتصال بمنصة بينانس Testnet بنجاح")
            else:
                if ENABLE_LOGGING:
                    logger.warning("⚠️ مفاتيح API غير متوفرة، استخدام الرصيد الافتراضي")
        except Exception as e:
            if ENABLE_LOGGING:
                logger.error(f"❌ فشل الاتصال ببينانس Testnet: {e}")

    def get_testnet_balance(self):
        """جلب الرصيد الفعلي من حساب الديمو في بينانس"""
        if not self.binance_client:
            if ENABLE_LOGGING:
                logger.warning("⚠️ استخدام الرصيد الافتراضي لعدم توفر اتصال ببينانس")
            return self.trade_config['initial_balance']
        
        try:
            # جلب معلومات الحساب من Testnet
            ...
            if usdt_balance > 0:
                if ENABLE_LOGGING:
                    logger.info(f"💰 الرصيد الفعلي من الديمو: {usdt_balance} USDT")
                return usdt_balance
            else:
                if ENABLE_LOGGING:
                    logger.warning(f"⚠️ الرصيد صفر في الديمو، استخدام الرصيد الافتراضي: {self.trade_config['initial_balance']}")
                return self.trade_config['initial_balance']
                
        except Exception as e:
            if ENABLE_LOGGING:
                logger.error(f"❌ خطأ في جلب الرصيد من الديمو: {e}")
            return self.trade_config['initial_balance']

    def get_current_testnet_balance(self):
        """جلب الرصيد الحالي من الديمو لتحديث الرصيد المحلي"""
        if not self.binance_client:
        ...
        try:
            ...
            if ENABLE_LOGGING:
                logger.error(f"❌ خطأ في جلب الرصيد الحالي من الديمو: {e}")
            return self.current_balance

    def update_balance_from_testnet(self):
        """تحديث الرصيد المحلي من حساب الديمو"""
    ...
    def send_telegram_message(self, message):
        """إرسال رسالة عبر التلغرام باستخدام python-telegram-bot"""
    ...
    def send_trade_notification(self, position, action, pnl_dollar=0, pnl_percent=0):
        ...
        if action == "OPEN":
            ...
            message = f"""
{emoji} **تم فتح صفقة جديدة (ديمو)** {emoji}
...
"""
            ...
            message = f"""
{emoji} **تم إغلاق الصفقة (ديمو)** {emoji}
...
"""
    ...
    def open_position(self, direction, signal_strength, row, details):
        """فتح صفقة جديدة في الديمو"""
        try:
            # تحديث الرصيد أولاً من الديمو
            self.update_balance_from_testnet()
            
            # حساب حجم المركز بناءً على الرصيد الفعلي من الديمو
            ...
            if ENABLE_LOGGING:
                ...
                logger.info(f"🟢 فتح {direction} #{position['id']} (ديمو) | قوة: {signal_strength:.1f}/10 {strength_emoji}")
                logger.info(f"📦 الحجم: ${position_value:.2f} ({size_percent:.1f}%) | الرصيد: ${self.current_balance:.2f}")
    ...
    def check_exit_conditions(self, row):
        ...
        for pos in [p for p in self.positions if p['status'] == 'OPEN']:
            ...
            if reason:
                ...
                if ENABLE_LOGGING:
                    emoji = "💰" if pnl_dollar > 0 else "💸"
                    logger.info(f"{emoji} إغلاق {pos['direction']} #{pos['id']} | {reason} | ${pnl_dollar:+.2f} ({pnl_percent*100:+.2f}%) | الرصيد: ${self.current_balance:.2f}")
...
"""
    ...
    def run_live_signal_check(self):
        ...
        try:
            # تحديث الرصيد من الديمو أولاً
            self.update_balance_from_testnet()
            
            self.fetch_binance_data(days=7)
...
def run_live_bot():
    """تشغيل البوت في الوضع الحي مع الديمو"""
    if ENABLE_LOGGING:
        logger.info("🚀 بدء تشغيل البوت المطور في وضع الديمو (Testnet)")
