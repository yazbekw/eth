...
class FuturesTradingBot:
    ...
    def calculate_indicators(self):
        ...
        self.data['ema_fast'] = self.data['ema_fast'].fillna(self.data['close'])
        self.data['ema_slow'] = self.data['ema_slow'].fillna(self.data['close'])
        self.data['ema_trend'] = self.data['ema_trend'].fillna(self.data['close'])
        self.data['macd'] = self.data['macd'].fillna(0)
        self.data['macd_signal'] = self.data['macd_signal'].fillna(0)
        
        print("✅ تم حساب المؤشرات الفنية للعقود الآجلة")
    
    def check_data_quality(self):
        """فحص جودة البيانات"""
        print(f"📊 جودة البيانات:")
        print(f"   إجمالي الأشرطة: {len(self.data)}")
        print(f"   البيانات الفارغة: {self.data.isnull().sum().sum()}")
        print(f"   أول سعر: {self.data['close'].iloc[0]:.2f}")
        print(f"   آخر سعر: {self.data['close'].iloc[-1]:.2f}")
        print(f"   نطاق RSI: {self.data['rsi'].min():.1f} - {self.data['rsi'].max():.1f}")
        print(f"   نطاق EMA السريع: {self.data['ema_fast'].min():.1f} - {self.data['ema_fast'].max():.1f}")
        print(f"   نطاق MACD: {self.data['macd'].min():.4f} - {self.data['macd'].max():.4f}")
    
    def debug_full_analysis(self):
        """تحليل كامل للنظام"""
        print("\n" + "="*50)
        print("🔍 تحليل كامل للنظام")
        print("="*50)
        ...
        # تحقق من الإعدادات
        print(f"\n⚙️ الإعدادات:")
        print(f"   min_conditions: {self.signal_config['min_conditions']}")
        print(f"   max_positions: {self.trade_config['max_positions']}")
        print(f"   الرافعة: {self.leverage}x")
        print(f"   وقف الخسارة: {self.trade_config['stop_loss']*100}%")
        print(f"   جني الأرباح: {self.trade_config['take_profit']*100}%")
        
        # اختبر بعض النقاط العشوائية
        print(f"\n🧪 اختبار نقاط عشوائية:")
        if len(self.data) > 100:
            test_indices = np.random.choice(range(50, min(1000, len(self.data))), 10, replace=False)
            
            signals_found = 0
            for i in test_indices:
                row = self.data.iloc[i]
                signal, strength = self.generate_signal(row)
                print(f"   النقطة {i}: {signal} (قوة: {strength}) - السعر: {row['close']:.2f}")
                if signal != 'HOLD':
                    signals_found += 1
            
            print(f"   📈 إشارات وجدت: {signals_found}/10")
        else:
    ...
    def generate_signal(self, row):
        """توليد إشارات التداول - مع ديباجنغ مفصل"""
        ...
        if nan_indicators:
            print(f"   ❌ مؤشرات بها NaN: {nan_indicators}")
            return 'HOLD', 0
        ...
        # 1. شرط RSI
        rsi_condition = row['rsi'] < self.indicator_config['rsi_oversold']
        if rsi_condition:
            long_conditions += 1

        # 2. شرط المتوسطات
        ema_condition = row['ema_fast'] > row['ema_slow']
        if ema_condition:
            long_conditions += 1
        ...
        # 4. شرط الاتجاه العام (إذا كان مفعل)
        if self.signal_config['use_trend_filter']:
            trend_condition = row['close'] > row['ema_trend']
            if trend_condition:
                long_conditions += 1
        ...
        # 1. شرط RSI
        rsi_short = row['rsi'] > self.indicator_config['rsi_overbought']
        if rsi_short:
            short_conditions += 1

        # 2. شرط المتوسطات
        ema_short = row['ema_fast'] < row['ema_slow']
        if ema_short:
            short_conditions += 1

        # 3. شرط MACD
        macd_short = row['macd'] < row['macd_signal']
        if macd_short:
            short_conditions += 1

        # 4. شرط الاتجاه العام (إذا كان مفعل)
        if self.signal_config['use_trend_filter']:
            trend_short = row['close'] < row['ema_trend']
            if trend_short:
                short_conditions += 1
        ...
        if long_conditions >= self.signal_config['min_conditions']:
            signal = 'LONG'
            strength = long_conditions
        elif short_conditions >= self.signal_config['min_conditions']:
            signal = 'SHORT' 
    ...
    def execute_backtest(self):
        ...
        print("\n🔄 بدء الباك تستينغ للعقود الآجلة...")
        print(f"💰 الرافعة: {self.leverage}x")
        print(f"🎯 شروط الدخول: {self.signal_config['min_conditions']}")
        ...
        for i, row in self.data.iterrows():
            ...
            if signal != 'HOLD':
                total_signals += 1
                print(f"\n🎯 إشارة #{total_signals} في {row['timestamp']}")
                print(f"   نوع: {signal} | قوة: {strength}")
                print(f"   السعر: {row['close']:.2f} | RSI: {row['rsi']:.1f}")
            ...
            elif signal in ['LONG', 'SHORT'] and open_positions >= self.trade_config['max_positions']:
                print(f"   ⚠️  تجاوز الحد الأقصى للصفقات ({self.trade_config['max_positions']})")
        
        print(f"\n📊 ملخص الباك تست:")
        print(f"   إجمالي الإشارات: {total_signals}")
        print(f"   الصفقات المنفذة: {executed_trades}")
        print(f"   الصفقات النهائية: {len(self.trades)}")
    ...
    def open_position(self, direction, row):
        ...
        print(f"      ⚠️  وقف الخسارة: {stop_loss:.2f}")
        print(f"      ✅ جني الأرباح: {take_profit:.2f}")
        print(f"      💀 التصفية: {liquidation_price:.2f}")
    ...
    def generate_report(self):
        ...
        if not self.trades:
            report = """
⚠️ لا توجد صفقات آجلة تم تنفيذها!

🔍 الأسباب المحتملة:
1. المؤشرات لا تصل لشروط الدخول (حتى مع min_conditions=1)
2. مشكلة في حساب المؤشرات
3. البيانات غير صالحة
4. الصفقات تصل للحد الأقصى بسرعة

📊 تحقق من:
- تقرير الديباجنغ أعلاه
- عدد الإشارات التي ظهرت
- إذا كانت الصفقات تفتح ثم تغلق فوراً
"""
            return report
...
def main():
    print("🚀 بدء تشغيل بوت العقود الآجلة مع الديباجنغ...")
    
    bot = FuturesTradingBot(TRADE_CONFIG, INDICATOR_CONFIG, SIGNAL_CONFIG)
    bot.fetch_binance_futures_data(days=30)  # اختصار المدة للاختبار
    
    # التحليل الشامل
    bot.debug_full_analysis()
