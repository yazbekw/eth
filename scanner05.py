...
class EnhancedMultiSourceDataFetcher:
    ...
    def _generate_coinex_signature(self, params: str) -> str:
        """توليد توقيع CoinEx"""
        signature = hmac.new(
            self._coinex_secret_key.encode('utf-8'),
            params.encode('utf-8'),
            hashlib.sha256
        ).hexdigest()
        return signature
    
    async def get_coinex_data(self, symbol: str, interval: str, limit: int = 50) -> Optional[Dict]:
        ...
        try:
            ...
            # تحويل الإطار الزمني ليتوافق مع CoinEx
            interval_mapping = {
                '1h': '60min',
                '4h': '240min',
                '1d': '1day',
            ...
            coinex_interval = interval_mapping.get(interval, '60min')
            
            url = f"https://api.coinex.com/v1/market/kline?market={symbol}&type={coinex_interval}&limit={limit}"
            ...
            logger.info(f"🔍 جلب البيانات من CoinEx لـ {symbol}...")
            ...
            logger.warning(f"⚠️ فشل جلب البيانات من CoinEx لـ {symbol}: {response.status_code}")
            return None
    ...
    def _process_coinex_kline_data(self, kline_data: List) -> List:
        ...
        for kline in kline_data:
            ...
            processed_data.append([
                kline[0] * 1000,  # timestamp (ms)
                str(kline[1]),    # open
                str(kline[3]),    # high
                str(kline[4]),    # low
                str(kline[2]),    # close
                str(kline[5]),    # volume
                kline[0] * 1000,  # close time
                str(kline[5]),    # quote asset volume
                "1",              # number of trades
