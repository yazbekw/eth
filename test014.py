...
SELL_CONFIDENCE_THRESHOLD = int(os.getenv("SELL_CONFIDENCE_THRESHOLD", "72"))
...
# إعدادات البيع المحسنة
SUPER_QUALITY_SELL_THRESHOLD = int(os.getenv("SUPER_QUALITY_SELL_THRESHOLD", "80"))
HIGH_QUALITY_SELL_THRESHOLD = int(os.getenv("HIGH_QUALITY_SELL_THRESHOLD", "75"))
...
logger = logging.getLogger("Enhanced_EMA_RSI_MACD_Strategy_v4")
...
# =============================================================================
# محرك الاستراتيجية المحسنة v4 مع تحسين متقدم للبيع
# =============================================================================

class EnhancedEmaRsiMacdStrategyV4:
    """استراتيجية محسنة v4 مع تحسينات متقدمة لأداء البيع"""
    
    def __init__(self, telegram_notifier: TelegramNotifier = None):
        self.name = "enhanced_ema_rsi_macd_v4"
    ...
    def execute_enhanced_paper_trading_v4(self, df: pd.DataFrame):
        """تنفيذ التداول الورقي المحسن v4"""
        
        logger.info("🚀 بدء التداول الورقي المحسن v4...")
    ...
    # =========================================================================
    # الباك-تستينغ المحسن v4
    # =========================================================================
    
    def run_enhanced_backtest_v4(self, df: pd.DataFrame) -> BacktestResult:
        """تشغيل الباك-تستينغ المحسن v4"""
        
        logger.info("🔍 بدء الباك-تستينغ المحسن v4...")
        ...
        # تنفيذ التداول المحسن v4
        self.execute_enhanced_paper_trading_v4(df_with_signals)
        ...
        # حساب النتائج المحسنة v4
        return self.calculate_enhanced_backtest_results_v4(df)
    
    def calculate_enhanced_backtest_results_v4(self, df: pd.DataFrame) -> BacktestResult:
        """حساب نتائج الباك-تستينغ المحسنة v4"""
    ...
    async def send_enhanced_telegram_report_v4(self, backtest_result: BacktestResult, df: pd.DataFrame):
        """إرسال تقرير مفصل v4 إلى التلغرام"""
        ...
        try:
            # 1. إرسال التقرير النصي المحسن v4
            report_text = self._generate_enhanced_report_text_v4(backtest_result)
            await self.telegram_notifier.send_message(report_text)
            
            # 2. إرسال الرسوم البيانية
            chart_buffer = self._create_enhanced_performance_chart_v4(df, backtest_result)
            if chart_buffer:
                chart_caption = f"📈 تحليل أداء الاستراتيجية المحسنة v4 - {SYMBOL} ({TIMEFRAME})"
                await self.telegram_notifier.send_photo(chart_buffer, chart_caption)
            
            # 3. إرسال تحليل البيع والشراء
            if self.trade_history:
                trade_analysis = self._generate_trade_analysis_v4(backtest_result)
                await self.telegram_notifier.send_message(trade_analysis)
                
        except Exception as e:
            logger.error(f"❌ خطأ في إرسال التقرير إلى التلغرام: {e}")
    
    def _generate_enhanced_report_text_v4(self, backtest_result: BacktestResult) -> str:
        """إنشاء نص التقرير المحسن v4 للتلغرام"""
        
        report_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        
        message = f"🎯 *تقرير استراتيجية المحسنة v4 - تحسين متقدم لأداء البيع*\n"
        message += "══════════════════════════════════════\n\n"
        
        message += f"⚙️ *الإعدادات المتقدمة v4:*\n"
        ...
        message += f"🎯 *مقاييس المخاطرة المحسنة v4:*\n"
        ...
        message += "══════════════════════════════════════\n"
        message += "⚡ *نظام التقييم v4 + تحسين متقدم للبيع + إعدادات متوازنة*"
        
        return message
    
    def _generate_trade_analysis_v4(self, backtest_result: BacktestResult) -> str:
        """إنشاء تحليل مفصل للبيع والشراء v4 مع توصيات"""
    
        message = "🔍 *تحليل مفصل للبيع والشراء v4:*\n"
        message += "────────────────────\n"
        ...
        message += f"🔽 *صفقات البيع المحسنة v4:*\n"
        ...
        message += f"📊 *مقارنة الأداء v4:*\n"
        message += f"• فرق النجاح: `{performance_gap:+.1f}%`\n"
        message += f"• فرق الربح: `${profit_gap:+.2f}`\n\n"
    
        # ✅ توصيات تحسين البيع بناء على النتائج v4
        message += f"🎯 *توصيات تحسين البيع v4:*\n"
    
        if sell['win_rate'] >= 80 and sell['total_pnl'] > 0:
            message += f"• الإعدادات الحالية ممتازة ✅\n"
            message += f"• يمكن زيادة عدوانية البيع قليلاً 📈\n"
            message += f"• الحفاظ على جودة الإشارات العالية 🎯\n"
        elif sell['win_rate'] >= 70:
            message += f"• الإعدادات الحالية جيدة جداً 👍\n"
            message += f"• تحسين تدريجي في شروط البيع 📊\n"
            message += f"• التركيز على البيع فائق الجودة 🚀\n"
        elif sell['win_rate'] >= 60:
            message += f"• تحسين عتبات البيع إلى 75% ⚙️\n"
            message += f"• زيادة شروط الجودة للبيع 🔧\n"
            message += f"• مراجعة إعدادات الوقف والجني 📉\n"
        else:
            message += f"• مراجعة شروط البيع الحالية 🔍\n"
            message += f"• زيادة عتبات الثقة للبيع 📈\n"
    ...
    def _create_enhanced_performance_chart_v4(self, df: pd.DataFrame, backtest_result: BacktestResult) -> BytesIO:
        """إنشاء رسم بياني محسن v4 للأداء"""
        try:
            fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(16, 12))
            fig.suptitle(f'تحليل الاستراتيجية المحسنة v4 - {SYMBOL}', 
            ...
            ax1.set_title('حركة السعر وإشارات التداول v4', fontname='DejaVu Sans', fontsize=12)
            ...
            if not trades_df.empty:
                ...
                ax2.set_title('توزيع أرباح البيع vs الشراء v4', fontname='DejaVu Sans', fontsize=12)
            ...
            if len(self.trade_history) > 0:
                ...
                ax3.set_title('تطور الرصيد v4', fontname='DejaVu Sans', fontsize=12)
            ...
            if quality_analysis:
                ...
                if categories:
                    ...
                    ax4.set_title('مقارنة أداء جودة البيع v4', fontname='DejaVu Sans', fontsize=12)
                    ax4.set_xticks(x)
...
async def main():
    """الوظيفة الرئيسية مع الاستراتيجية المحسنة v4"""
    
    logger.info("🚀 بدء تشغيل الاستراتيجية المحسنة v4 مع تحسين متقدم لأداء البيع")
    ...
    # تشغيل الاستراتيجية المحسنة v4
    strategy = EnhancedEmaRsiMacdStrategyV4(telegram_notifier)
    
    # الباك-تستينغ المحسن v4
    backtest_result = strategy.run_enhanced_backtest_v4(df)
    
    # إرسال التقرير المحسن v4 إلى التلغرام
    await strategy.send_enhanced_telegram_report_v4(backtest_result, df)
    
    # حفظ النتائج في ملف
    trades_df = pd.DataFrame(strategy.trade_history)
    if not trades_df.empty:
        filename = f'enhanced_v4_trades_{SYMBOL}_{TIMEFRAME}_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv'
        trades_df.to_csv(filename, index=False)
        logger.info(f"💾 تم حفظ سجل الصفقات في {filename}")
    
    logger.info("✅ اكتمل تشغيل الاستراتيجية المحسنة v4 بنجاح")
