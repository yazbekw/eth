...
class AdvancedCryptoBot:
    ...
    def generate_signal(self, row):
        ...
        if (buy_conditions >= min_conditions and 
            ...
            signal_strength >= self.signal_config.get('min_signal_strength', 3)):
            signal = 'BUY'
    ...
    def execute_backtest(self, min_strength=None):
        """تنفيذ الباك تستينغ مع قوة الإشارة"""
        if min_strength is not None:
            self.signal_config['min_signal_strength'] = min_strength
        
        print(f"🔄 بدء الباك تستينغ - قوة الإشارة: {self.signal_config['min_signal_strength']}+")
    ...
    def test_all_strength_levels(self):
        """اختبار جميع مستويات قوة الإشارة من 1 إلى 10"""
        print("🧪 بدء اختبار جميع مستويات قوة الإشارة...")
        print("=" * 60)
        ...
        for strength_level in range(1, 11):
            ...
            # تشغيل الباك تستينغ بمستوى القوة الحالي
            self.execute_backtest(strength_level)
            ...
            print(f"📊 قوة {strength_level}: {total_trades} صفقات | نسبة الربح: {win_rate:.1f}% | إجمالي الربح: {total_pnl:+.2f}%")
        
        # حفظ النتائج
        self.signal_strength_results = results
        
        # عرض أفضل النتائج
        self.show_optimal_strength(results)
        
        return results
    
    def show_optimal_strength(self, results):
        """عرض أفضل مستويات قوة الإشارة"""
        if not results:
            return
        ...
        if not valid_results:
            print("⚠️ لا توجد نتائج كافية لتحديد المستوى الأمثل")
            return
        
        # إيجاب أفضل نتيجة بناء على نسبة الربح وعدد الصفقات
        best_result = max(valid_results, 
                         key=lambda x: (x['win_rate'] * min(x['total_trades'], 10) / 10))
        
        print("\n🎯 أفضل مستوى قوة إشارة:")
        print(f"   المستوى: {best_result['strength_level']}")
        ...
        # تحديث الإعدادات بأفضل مستوى
        self.signal_config['min_signal_strength'] = best_result['strength_level']
        print(f"✅ تم تحديث الإعدادات إلى مستوى القوة {best_result['strength_level']}")
...
def main():
    print("🚀 بدء تشغيل البوت المتقدم مع قوة الإشارة المتدرجة...")
    print("=" * 60)
    ...
    print("\n🧪 بدء اختبار مستويات قوة الإشارة من 1 إلى 10...")
    bot.test_all_strength_levels()
    
    print("\n📈 تشغيل الباك تستينغ النهائي بالمستوى الأمثل...")
