...
SIGNAL_CONFIG = {
    ...
    'exact_signal_strength': 5  # ⭐ قوة إشارة ثابتة = 5 بالضبط
}
...
class ExactStrengthCryptoBot:
    ...
    def calculate_exact_signal_strength(self, buy_conditions, sell_conditions, row):
        """حساب قوة الإشارة لضمان الحصول على 5 بالضبط"""
        # إرجاع 5 ثابتة دائماً مع التحقق من الشروط الأساسية
        base_conditions = buy_conditions if buy_conditions > sell_conditions else sell_conditions
        
        # التحقق من الحد الأدنى من الشروط
        if base_conditions < self.signal_config['min_conditions']:
            return 0  # لا توجد إشارة
        
        # التحقق من جودة الإشارة الأساسية
        rsi_ok = (buy_conditions > 0 and row['rsi'] < 40) or (sell_conditions > 0 and row['rsi'] > 60)
        trend_ok = (buy_conditions > 0 and row['ema_fast'] > row['ema_slow']) or (sell_conditions > 0 and row['ema_fast'] < row['ema_slow'])
        volume_ok = row['volume'] > row['volume_ma'] * 0.8
        
        if rsi_ok and trend_ok and volume_ok:
            return 5  # ⭐ قوة 5 بالضبط
        else:
            return 0  # لا توجد إشارة
    
    def generate_signal(self, row):
        """توليد إشارات تداول بقوة 5 بالضبط"""
        ...
        # ⭐ حساب قوة الإشارة - 5 بالضبط أو لا شيء
        signal_strength = self.calculate_exact_signal_strength(buy_conditions, sell_conditions, row)
        
        # اتخاذ القرار النهائي - فقط عندما تكون القوة 5 بالضبط
        signal = 'HOLD'
        
        if signal_strength == 5:  # ⭐ شرط القوة 5 بالضبط
    ...
    def execute_backtest(self):
        """تنفيذ الباك تستينغ مع قوة الإشارة 5 بالضبط"""
        print(f"🔄 بدء الباك تستينغ - ⭐ قوة الإشارة: 5 بالضبط فقط")
        ...
        for i, row in self.data.iterrows():
            ...
            # ⭐ الشرط الجديد: يجب أن تكون القوة 5 بالضبط
        ...
        print(f"✅ تم الانتهاء - {len(self.trades)} صفقة من أصل {signals_count} إشارة قوة 5")
    ...
    def open_position(self, direction, signal_strength, row, details):
        ...
        print(f"   ⭐ قوة: 5/5 بالضبط | 📊 {details}")
...
⭐ إستراتيجية قوة الإشارة 5 بالضبط
...
def main():
    print("🚀 بدء تشغيل البوت - ⭐ قوة الإشارة 5 بالضبط فقط")
    print("=" * 60)
    
    if not TELEGRAM_CONFIG['bot_token'] or not TELEGRAM_CONFIG['chat_id']:
        print("⚠️  تنبيه: مفاتيح التلغرام غير متوفرة")
    
    bot = ExactStrengthCryptoBot(TRADE_CONFIG, INDICATOR_CONFIG, SIGNAL_CONFIG)
    bot.fetch_binance_data(days=30)
    
    print("\n📈 تشغيل الباك تستينغ بقوة الإشارة 5 بالضبط...")
