...
# =============================================================================
# إعدادات التداول المحسنة للبيع - إصدار أكثر مرونة
# =============================================================================
...
# عتبات منفصلة للشراء والبيع - أكثر مرونة للبيع
BUY_CONFIDENCE_THRESHOLD = int(os.getenv("BUY_CONFIDENCE_THRESHOLD", "70"))
SELL_CONFIDENCE_THRESHOLD = int(os.getenv("SELL_CONFIDENCE_THRESHOLD", "72"))  # مخفضة
SELL_PREMIUM_THRESHOLD = int(os.getenv("SELL_PREMIUM_THRESHOLD", "76"))  # مخفضة
SELL_QUALITY_THRESHOLD = int(os.getenv("SELL_QUALITY_THRESHOLD", "70"))  # مخفضة
...
class EnhancedSellStrategy:
    ...
    def __init__(self, telegram_notifier: TelegramNotifier = None):
        ...
        self.df_global = None  # تخزين البيانات العالمية
    ...
    def calculate_enhanced_results(self, df: pd.DataFrame) -> BacktestResult:
        ...
        # إنشاء DataFrame من سجل التداول مع التعامل مع الأعمدة المفقودة
        trades_data = []
    ...
    def run_enhanced_backtest(self, df: pd.DataFrame) -> BacktestResult:
        ...
        logger.info("🔍 بدء الباك-تستينغ المحسن للبيع v6...")
    ...
    def _generate_enhanced_report_text(self, backtest_result: BacktestResult) -> str:
        ...
        message = f"🎯 تقرير استراتيجية المحسنة v6 - إصدار أكثر مرونة\n"
        message += "══════════════════════════════════════\n\n"
        
        message += f"⚙️ الإعدادات المتقدمة v6:\n"
        ...
        message += f"🎯 مقاييس المخاطرة المحسنة v6:\n"
        ...
        message += "══════════════════════════════════════\n"
        message += "⚡ نظام التقييم v6 + شروط بيع مرنة + إعدادات متوازنة\n\n"
        
        message += f"🔍 تحليل مفصل للبيع والشراء v6:\n"
        message += "────────────────────\n"
        ...
        # تحليل البيع
        message += f"🔽 صفقات البيع المحسنة v6:\n"
        ...
        # توصيات
        message += f"🎯 توصيات تحسين البيع v6:\n"
        if buy_analysis['total_sell_trades'] == 0:
            message += f"• تم تخفيف شروط البيع بشكل كبير ✅\n"
            message += f"• إضافة نمط بيع حجمي جديد 📊\n"
            message += f"• تقليل عتبات الثقة والجودة 📉\n"
        elif buy_analysis['sell_win_rate'] < 50:
            message += f"• البيع يعمل ولكن يحتاج تحسينات طفيفة ⚠️\n"
            message += f"• تحليل أداء فئات البيع المختلفة 📈\n"
        ...
        if buy_analysis['total_sell_trades'] > 0 and buy_analysis['sell_win_rate'] > 50 and buy_analysis['sell_total_pnl'] > 0:
            ...
            final_msg = "⚠️ استراتيجية البيع المحسنة تعمل ولكن تحتاج تحسينات"
        else:
...
async def main():
    ...
    logger.info("🚀 بدء تشغيل استراتيجية البيع المحسنة v6 - إصدار أكثر مرونة")
    ...
    if strategy.trade_history:
        ...
        filename = f'enhanced_sell_trades_v6_{SYMBOL}_{TIMEFRAME}_{datetime.now().strftime("%Y%m%d_%H%M")}.csv'
