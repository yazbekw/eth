...
CONFIDENCE_THRESHOLD_SINGLE = 75  # عتبة الإشارة الواحدة
CONFIDENCE_THRESHOLD_MULTIPLE = 65  # عتبة الإشارات المتعددة
MIN_STRATEGY_CONFIDENCE = 50  # أقل ثقة للاستراتيجيات المحتسبة
ANALYSIS_REPORT_THRESHOLD = 20  # عتبة منخفضة للتحليل التفصيلي
...
# =============================================================================
# محرك الإشارات المتقدم مع نظام التقارير التفصيلية
# =============================================================================
...
class AdvancedSignalEngine:
    ...
    async def generate_detailed_market_report(self) -> Dict[str, Any]:
        """توليد تقرير تفصيلي شامل لجميع العملات المدعومة"""
        logger.info("📊 بدء توليد التقرير التفصيلي للسوق...")
        ...
        for coin_key, coin_data in SUPPORTED_COINS.items():
            try:
                logger.info(f"🔍 تحليل {coin_key} للتقرير التفصيلي...")
                analysis = await self.analyze_coin(coin_key, coin_data['binance_symbol'])
                
                if analysis.get('success'):
                    market_report["analysis"][coin_key] = analysis
                    ...
                    if buy_count > 0 and sell_count > 0:
                        market_report["summary"]["total_conflicts"] += 1
                    elif buy_count > 0:
                        market_report["summary"]["total_buy_signals"] += 1
                    elif sell_count > 0:
                        market_report["summary"]["total_sell_signals"] += 1
                    
                    # تصنيف قوة الإشارات
                    if analysis.get('confidence', 0) >= CONFIDENCE_THRESHOLD_SINGLE:
                        market_report["summary"]["strong_signals"] += 1
                    elif any(strategy_data.get('confidence', 0) >= ANALYSIS_REPORT_THRESHOLD 
                            for strategy_data in strategies_analysis.values()):
                        market_report["summary"]["weak_signals"] += 1
                    else:
                        market_report["summary"]["no_signals"] += 1
                        
            except Exception as e:
                logger.error(f"❌ خطأ في تحليل {coin_key} للتقرير: {e}")
                market_report["analysis"][coin_key] = {
                    "success": False,
                    "error": str(e),
                    "signal": "none",
                    "confidence": 0
        ...
        logger.info("✅ تم إنشاء التقرير التفصيلي بنجاح")
        return market_report

# =============================================================================
# نظام التقارير التفصيلية المحسن
# =============================================================================

class DetailedReportGenerator:
    """مولد التقارير التفصيلية المحسن"""
    
    def __init__(self, notifier):
        self.notifier = notifier
    
    def create_detailed_market_report(self, market_report: Dict[str, Any]) -> str:
        """إنشاء تقرير تفصيلي للسوق بتنسيق Markdown"""
        try:
            report_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            
            message = f"📊 **تقرير السوق التفصيلي - تحليل متعدد الاستراتيجيات**\n"
            message += "═" * 50 + "\n"
            message += f"⏰ **وقت التقرير:** `{report_time}`\n"
            message += f"📈 **الإطار الزمني:** `{market_report['timeframe']}`\n"
            message += f"💰 **إجمالي العملات:** `{market_report['total_coins']}`\n\n"
            
            # ملخص الإحصائيات
            summary = market_report['summary']
            message += "📈 **ملخص السوق:**\n"
            message += f"🟢 **إشارات شراء:** `{summary['total_buy_signals']}` عملة\n"
            message += f"🔴 **إشارات بيع:** `{summary['total_sell_signals']}` عملة\n"
            message += f"⚖️ **تضاربات:** `{summary['total_conflicts']}` عملة\n"
            message += f"💪 **إشارات قوية:** `{summary['strong_signals']}` عملة\n"
            message += f"📉 **إشارات ضعيفة:** `{summary['weak_signals']}` عملة\n"
            message += f"⚪ **بدون إشارات:** `{summary['no_signals']}` عملة\n\n"
            
            message += "🔍 **التفاصيل حسب العملة:**\n"
            message += "─" * 40 + "\n"
            
            # تفاصيل كل عملة
            for coin_key, analysis in market_report['analysis'].items():
                if not analysis.get('success'):
                    continue
                    
                coin_data = SUPPORTED_COINS.get(coin_key, {})
                coin_name = coin_data.get('name', coin_key.upper())
                ...
                for strategy_name, strategy_data in strategies_analysis.items():
                    ...
                    strategy_display_name = self._get_strategy_display_name(strategy_name)
                    
                    if signal == 'BUY':
                        buy_strategies.append(f"{strategy_display_name} ({confidence}%)")
                    elif signal == 'SELL':
                        sell_strategies.append(f"{strategy_display_name} ({confidence}%)")
                    else:
                        if confidence >= 20:  # فقط عرض الاستراتيجيات ذات الثقة المعقولة
                            neutral_strategies.append(f"{strategy_display_name} ({confidence}%)")
                
                # عرض نتائج الاستراتيجيات
                if buy_strategies:
                    message += f"🟢 **شراء:** {', '.join(buy_strategies)}\n"
                if sell_strategies:
                    message += f"🔴 **بيع:** {', '.join(sell_strategies)}\n"
                if neutral_strategies:
                    message += f"⚪ **محايد:** {', '.join(neutral_strategies)}\n"
            ...
            for coin_key, analysis in market_report['analysis'].items():
                if (analysis.get('success') and 
                    analysis.get('signal') != 'none' and 
                    analysis.get('confidence', 0) >= CONFIDENCE_THRESHOLD_SINGLE):
                    
                    coin_name = SUPPORTED_COINS.get(coin_key, {}).get('name', coin_key.upper())
                    signal_emoji = "🟢" if analysis['signal'] == 'BUY' else "🔴"
                    strong_signals.append(f"{signal_emoji} **{coin_name}**: {analysis['signal']} ({analysis['confidence']}%)")
            
            if strong_signals:
                message += "✅ **إشارات قوية للتنفيذ:**\n"
                for signal in strong_signals:
                    message += f"• {signal}\n"
            else:
                message += "⏳ **لا توجد إشارات قوية للتنفيذ حالياً**\n"
            
            message += "\n"
            message += "📋 **معلومات التقنية:**\n"
            message += f"• **عتبة الإشارة القوية:** `{CONFIDENCE_THRESHOLD_SINGLE}%`\n"
            message += f"• **عتبة التحليل الأدنى:** `{ANALYSIS_REPORT_THRESHOLD}%`\n"
            message += f"• **الاستراتيجيات:** EMA+RSI+MACD, الانزياح الحجمي, الأموال الذكية\n"
            
            message += "═" * 50 + "\n"
            message += "⚡ **المحرك المتقدم للإشارات - التقارير التفصيلية**"
            
            return message
            
        except Exception as e:
            logger.error(f"❌ خطأ في إنشاء التقرير التفصيلي: {e}")
            return f"❌ خطأ في إنشاء التقرير التفصيلي: {e}"
    ...
    async def send_detailed_report(self, market_report: Dict[str, Any]) -> bool:
        """إرسال التقرير التفصيلي"""
        try:
            report_message = self.create_detailed_market_report(market_report)
            ...
            logger.error(f"❌ خطأ في إرسال التقرير التفصيلي: {e}")
            return False
    ...
    async def _send_single_message(self, message: str) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                logger.info("✅ تم إرسال التقرير التفصيلي بنجاح")
...
class TelegramNotifier:
    ...
    async def send_heartbeat(self, executor_connected: bool, signals_count: int = 0, 
        ...
        try:
            ...
            message += f"📈 *التقارير التفصيلية:* `{system_stats['detailed_reports_sent']}`\n\n"
...
class ExecutorBotClient:
    """عميل للتواصل مع بوت التنفيذ - النسخة المحدثة والمحسنة"""
    ...
    async def send_trade_signal(self, signal_data: Dict[str, Any]) -> bool:
        """إرسال إشارة تداول إلى البوت المنفذ - النسخة المحسنة"""
        ...
        try:
            ...
            # بناء payload متوافق مع توقعات البوت المنفذ
            payload = {
                "signal": signal_data,
                "timestamp": time.time(),
                "source": "advanced_signal_generator_v2",
                "system_stats": {
            ...
            logger.info(f"🔄 إرسال إشارة إلى البوت المنفذ: {signal_data['coin']} - {signal_data['action']} - {signal_data['confidence']}%")
    ...
    def _validate_signal_data(self, signal_data: Dict[str, Any]) -> bool:
        """التحقق من صحة بيانات الإشارة قبل الإرسال"""
        required_fields = ['coin', 'symbol', 'action', 'confidence', 'price']
        
        for field in required_fields:
            if field not in signal_data:
                logger.error(f"❌ حقل مطلوب مفقود في الإشارة: {field}")
                return False
        ...
        try:
            ...
            if confidence <= 0 or confidence > 100:
                logger.error(f"❌ قيمة ثقة غير صالحة: {confidence}")
                return False
                
            if price <= 0:
                logger.error(f"❌ سعر غير صالح: {price}")
                return False
                
            if signal_data['action'] not in ['BUY', 'SELL']:
                logger.error(f"❌ إجراء غير صالح: {signal_data['action']}")
                return False
                
        except (ValueError, TypeError) as e:
            logger.error(f"❌ خطأ في تحويل أنواع البيانات: {e}")
            return False
...
report_generator = DetailedReportGenerator(notifier)

# =============================================================================
# المهام الأساسية مع نظام التقارير التفصيلية
# =============================================================================
...
async def advanced_market_scanner_task():
    """المهمة الرئيسية للمسح الضوئي المتقدم مع التقارير التفصيلية"""
    global recent_analysis, last_detailed_report_time
    logger.info("🚀 بدء مهمة مسح السوق المتقدم كل 15 دقيقة مع التقارير التفصيلية")
    ...
    while True:
        try:
            ...
            # إرسال التقارير التفصيلية
            current_time = time.time()
            if current_time - last_detailed_report_time >= DETAILED_REPORT_INTERVAL or signals_found > 0:
                logger.info("📊 إنشاء التقرير التفصيلي للسوق...")
                market_report = await signal_engine.generate_detailed_market_report()
                await report_generator.send_detailed_report(market_report)
                last_detailed_report_time = current_time
            
            # إرسال إشارات التداول القوية
            if signals_found > 0:
                await send_unified_alert(scan_results)
                
                for result in scan_results:
                    try:
                        # 🔍 بناء إشارة تداول مع التحقق من الأنواع
                        analysis = result['analysis']
                        
                        trade_signal = {
                            "coin": str(result['coin']),
                            "symbol": str(result['coin_data']['binance_symbol']),
                            "action": str(analysis["signal"]),
                            "timeframe": str(TIMEFRAME),
                            "price": float(analysis["current_price"]),
                            "confidence": float(analysis["confidence"]),
                            "winning_strategies": int(analysis["winning_strategies"]),
                            "total_strategies": int(analysis["total_strategies"]),
                            "reasons": list(analysis["reasons"]),
                            "strategies_analysis": dict(analysis["strategies_analysis"]),
                            "data_source": str(analysis.get("data_source", "unknown"))
                        }
                        
                        logger.info(f"🔍 [إرسال] إعداد إشارة: {trade_signal['coin']} - {trade_signal['action']} - {trade_signal['confidence']}%")
                        
                        # إرسال الإشارة
                        success = await executor_client.send_trade_signal(trade_signal)
                        
                        if success:
                            logger.info(f"🚀 تم إرسال الإشارة بنجاح إلى البوت المنفذ")
                        else:
                            logger.error(f"💥 فشل في إرسال الإشارة للمنفذ")
                            
                        await asyncio.sleep(1)
                        
                    except Exception as e:
                        logger.error(f"❌ خطأ في إعداد الإشارة لـ {result['coin']}: {e}")
                        continue
...
@app.get("/detailed-report")
async def generate_detailed_report():
    """توليد وإرسال تقرير تفصيلي يدوي"""
    try:
        logger.info("📊 بدء توليد التقرير التفصيلي يدوياً...")
        market_report = await signal_engine.generate_detailed_market_report()
        success = await report_generator.send_detailed_report(market_report)
        
        return {
            "status": "success" if success else "error",
            "message": "تم إرسال التقرير التفصيلي بنجاح" if success else "فشل إرسال التقرير التفصيلي",
            "report_generated": True,
            "coins_analyzed": len(market_report['analysis']),
            "timestamp": datetime.now().isoformat()
        }
    except Exception as e:
        logger.error(f"❌ خطأ في توليد التقرير التفصيلي: {e}")
        return {"status": "error", "message": str(e)}

@app.get("/market-analysis")
async def get_market_analysis():
    """الحصول على تحليل السوق الحالي بدون إرسال"""
    try:
        market_report = await signal_engine.generate_detailed_market_report()
        
        # تبسيط البيانات للعرض في API
        simplified_analysis = {}
        for coin_key, analysis in market_report['analysis'].items():
            if analysis.get('success'):
                simplified_analysis[coin_key] = {
                ...
                for strategy_name, strategy_data in analysis.get('strategies_analysis', {}).items():
                    simplified_analysis[coin_key]['strategies'][strategy_name] = {
                        'signal': strategy_data.get('signal', 'none'),
                        'confidence': strategy_data.get('confidence', 0)
                    }
        ...
        return {
            "status": "success",
            "timestamp": market_report['timestamp'],
            "summary": market_report['summary'],
...
async def startup_event():
    logger.info("🚀 بدء تشغيل مولد الإشارات المتقدم مع التقارير التفصيلية")
    ...
    logger.info(f"📋 فاصل التقارير التفصيلية: {DETAILED_REPORT_INTERVAL} ثانية")
    logger.info(f"💡 الوضع: تقارير تفصيلية شاملة")
