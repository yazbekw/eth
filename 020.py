...
TRADE_CONFIG = {
    ...
    'base_stop_loss': 0.015,  # تعديل وقف الخسارة للرافعة
    'base_take_profit': 0.045,  # تعديل جني الأرباح للرافعة
    'base_position_size': 0.25,
    'max_positions': 4,
    'paper_trading': True,
    'use_trailing_stop': ENABLE_TRAILING_STOP,
    'trailing_stop_percent': 0.01,  # تعديل للتداول المتعقب
    'trailing_activation': 0.015,
    'max_trade_duration': 48,
    'atr_multiplier_sl': 1.5,  # تعديل مضاعف ATR للرافعة
    'atr_multiplier_tp': 3.0,
    'atr_period': 14,
    'support_resistance_window': 20,
    'peak_hours': [0, 4, 8, 12, 16, 20],
    'min_volume_ratio': 1.2,
    'market_type': 'FUTURES',  # إضافة نوع السوق
    'margin_mode': 'ISOLATED'  # أو 'CROSS'
}
...
class AdvancedCryptoBot:
    ...
    def fetch_binance_data(self, days=60):
        ...
        try:
            ...
            if ENABLE_LOGGING:
                logger.info(f"جلب {required_candles} شمعة من العقود الآجلة {symbol} ({interval})")
    ...
    def generate_sample_data(self, days):
        """بيانات عينة آمنة للعقود الآجلة"""
        try:
            ...
            np.random.seed(42)
            price = 3000.0  # سعر أعلى محتمل للعقود الآجلة
            prices = []
            ...
            self.data = pd.DataFrame({
                ...
                'close': prices,
                'volume': [abs(np.random.normal(10000, 3000)) for _ in prices]  # حجم أعلى للعقود الآجلة
            })
            self.calculate_indicators()
            if ENABLE_LOGGING:
                logger.info(f"تم إنشاء {len(self.data)} شمعة عينة للعقود الآجلة")
        except Exception as e:
            if ENABLE_LOGGING:
    ...
    def calculate_liquidation_price(self, direction, entry_price, stop_loss, leverage):
        """حساب سعر التصفية التقريبي"""
        try:
            if direction == 'BUY':
                # للشراء: سعر التصفية ≈ سعر الدخول * (1 - 1/الرافعة المالية + هامش الحماية)
                buffer = 0.02  # هامش حماية 2%
                liquidation = entry_price * (1 - (1/leverage) + buffer)
            else:
                # للبيع: سعر التصفية ≈ سعر الدخول * (1 + 1/الرافعة المالية - هامش الحماية)
                buffer = 0.02  # هامش حماية 2%
                liquidation = entry_price * (1 + (1/leverage) - buffer)
    ...
    def open_position(self, direction, signal_strength, row, details):
        """فتح صفقة جديدة في العقود الآجلة"""
        try:
            # حساب حجم المركز مع مراعاة الرافعة المالية
            ...
            if ENABLE_DYNAMIC_POSITION_SIZING:
                ...
                position_value = self.current_balance * base_size * size_factor * leverage
            else:
                position_value = self.current_balance * base_size * leverage

            entry = row['close']
            
            # حساب وقف الخسارة وجني الأرباح مع مراعاة الرافعة
            base_sl = self.trade_config['base_stop_loss'] / leverage
            base_tp = self.trade_config['base_take_profit'] / leverage
            ...
            # حساب حجم العقد (تقريبي)
            contract_size = position_value / entry
            ...
            position = {
                ...
                'status': 'OPEN',
                'type': 'FUTURES_PAPER' if self.paper_trading else 'FUTURES_REAL',
                ...
                'liquidation_price': self.calculate_liquidation_price(direction, entry, sl, leverage)
            }
            ...
            if ENABLE_LOGGING:
                ...
                size_percent = (position_value / (self.current_balance * leverage)) * 100
                logger.info(f"فتح {direction} #{position['id']} | رافعة: {leverage}x | قوة: {signal_strength:.1f}/10 {strength_emoji}")
                logger.info(f"الحجم: ${position_value:.2f} ({size_percent:.1f}% من الرصيد) | {details}")
                
        except Exception as e:
            if ENABLE_LOGGING:
                logger.error(f"خطأ فتح الصفقة في العقود الآجلة: {e}")
    ...
    def execute_backtest(self):
        ...
        for data, name in datasets:
            if ENABLE_LOGGING:
                logger.info(f"باك تست العقود الآجلة: {name} ({len(data)} شمعة)")
    ...
    def validate_calculations(self):
        ...
        for trade in self.trades:
            ...
            if trade['direction'] == 'BUY':
                ...
                calculated_pnl_percent = (trade['entry_price'] - trade['exit_price']) / trade['exit_price']
    ...
    def analyze_trades(self):
        ...
        # إحصائيات الرافعة المالية
        avg_leverage = df['leverage'].mean() if 'leverage' in df.columns else 1
        max_leverage = df['leverage'].max() if 'leverage' in df.columns else 1
    ...
    def generate_report(self):
        """توليد تقرير الأداء الأساسي للعقود الآجلة"""
        if not self.trades:
        ...
        # حساب إحصائيات إضافية للعقود الآجلة
        total_leverage_used = analysis['avg_leverage']
        max_leverage = analysis['max_leverage']

        report = f"""
📈 تقرير الأداء النهائي - العقود الآجلة
{'='*50}
...
• متوسط الرافعة المالية: {total_leverage_used:.1f}x
• أقصى رافعة مستخدمة: {max_leverage}x
{'='*50}
...
"""
    ...
    def generate_detailed_report(self):
        ...
        report = f"""
📊 **تقرير أداء مفصل - بوت العقود الآجلة**
{'='*60}
...
• متوسط الرافعة: {analysis['avg_leverage']:.1f}x
{'='*60}
**🔍 تحليل أسباب الإغلاق:**
"""
...
def main():
    if ENABLE_LOGGING:
        logger.info("بدء تشغيل بوت العقود الآجلة مع نظام قوة الإشارة المحسن")
