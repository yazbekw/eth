...
# =============================================================================
# محرك الاستراتيجية المحسنة v3 مع تحسين أداء البيع
# =============================================================================

class EnhancedEmaRsiMacdStrategyV3:
    """استراتيجية محسنة v3 مع تحسينات متقدمة لأداء البيع"""
    ...
    def add_smart_filters_v3(self, df: pd.DataFrame) -> pd.DataFrame:
        """إضافة عوامل تصفية ذكية v3 مع إنشاء atr_percent"""
    ...
    def risk_adjusted_scoring(self, df: pd.DataFrame) -> pd.DataFrame:
        """نظام تقييم معدل حسب المخاطرة مع معالجة آمنة"""
        
        # التحقق من وجود الأعمدة المطلوبة
        required_columns = ['atr_percent', 'rsi_volatility', 'score_v3']
        missing_columns = [col for col in required_columns if col not in df.columns]
        
        if missing_columns:
            logger.warning(f"⚠️ أعمدة مفقودة في risk_adjusted_scoring: {missing_columns}")
            return df
    ...
    def _generate_enhanced_report_text_v3(self, backtest_result: BacktestResult) -> str:
        ...
        message = f"🎯 *تقرير استراتيجية المحسنة v3 - تحسين أداء البيع*\n"
        message += "══════════════════════════════════════\n\n"
        ...
        message += "══════════════════════════════════════\n"
        message += "⚡ *نظام التقييم v3 + تحسين أداء البيع + إعدادات متوازنة*"
...
async def main():
    ...
    logger.info("🚀 بدء تشغيل الاستراتيجية المحسنة v3 مع تحسين أداء البيع")
