...
SELL_CONFIDENCE_THRESHOLD = int(os.getenv("SELL_CONFIDENCE_THRESHOLD", "78"))
SELL_PREMIUM_THRESHOLD = int(os.getenv("SELL_PREMIUM_THRESHOLD", "82"))
SELL_QUALITY_THRESHOLD = int(os.getenv("SELL_QUALITY_THRESHOLD", "80"))
...
logger = logging.getLogger("Enhanced_Sell_Strategy_v5_Fixed")
...
# =============================================================================
# استراتيجية الانزياح الحجمي المحسنة للبيع - الإصدار المصحح
# =============================================================================

class EnhancedSellStrategy:
    """استراتيجية محسنة تركز على تحسين أداء البيع - الإصدار المصحح"""
    
    def __init__(self, telegram_notifier: TelegramNotifier = None):
        self.name = "enhanced_sell_strategy_v5_fixed"
    ...
    def _generate_enhanced_report_text(self, backtest_result: BacktestResult) -> str:
        ...
        message = f"🎯 تقرير استراتيجية المحسنة v5 - الإصدار المصحح\n"
        message += "══════════════════════════════════════\n\n"
...
async def main():
    ...
    logger.info("🚀 بدء تشغيل استراتيجية البيع المحسنة v5 - الإصدار المصحح")
    ...
    if strategy.trade_history:
        ...
        filename = f'enhanced_sell_trades_v5_fixed_{SYMBOL}_{TIMEFRAME}_{datetime.now().strftime("%Y%m%d_%H%M")}.csv'
