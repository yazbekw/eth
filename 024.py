...
CONFIDENCE_THRESHOLD = 6  # عتبة الثقة للإشارات (من 1-10)
...
# ====================== الإعدادات الأساسية ======================
TRADE_CONFIG = {
    'symbols': ["BTCUSDT", "XRPUSDT", "SOLUSDT", "ADAUSDT"],
    'timeframe': '4h',
    'initial_balance': 200,
    'leverage': 10,
...
💡 **التوصية:** {'الدخول في صفقة شراء مع إدارة المخاطر' if action == 'BUY' else 'الدخول في صفقة بيع مع إدارة المخاطر'}
    ...
    def _build_heartbeat_message(self) -> str:
        ...
        total_buy_signals = system_stats["buy_signals"]
        total_sell_signals = system_stats["sell_signals"]
        
        message = f"""
💓 **نبضة النظام - ماسح الإشارات المتقدم**
...
📊 **إحصائيات الإشارات:**
• 🟢 **إشارات شراء:** `{total_buy_signals}`
• 🔴 **إشارات بيع:** `{total_sell_signals}`
• ⚖️ **النسبة:** `{total_buy_signals}:{total_sell_signals}`
...
📅 **آخر إشارة:** `{system_stats["last_signal_time"] or "لا توجد"}`
...
✅ **الحالة:** النظام يعمل بشكل طبيعي
    ...
    def _build_startup_message(self) -> str:
        ...
        message = f"""
🚀 **بدء تشغيل ماسح الإشارات المتقدم v2.0**

🕒 **وقت البدء:** `{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}`
🔧 **الإصدار:** `2.0.0 - النظام المحسن`
...
• 🔍 مسح متعدد الرموز
• 🎯 كشف القمم والقيعان
• 📡 إرسال تلقائي للإشارات
• 🔔 إشعارات فورية
• 📊 إحصائيات مفصلة

✅ **جاهز للبدء في مسح الأسواق...**
        """
...
class AdvancedCryptoScanner:
    ...
    def generate_signal(self, symbol: str, row):
        ...
        try:
            ...
            safe_log_error(f"خطأ في توليد الإشارة لـ {symbol}: {e}")
        ...
        try:
            ...
            if signal in ['BUY', 'SELL'] and strength >= CONFIDENCE_THRESHOLD:
                ...
                if ENABLE_SIGNAL_SENDING:
                    ...
                    if sent:
                        safe_log_info(f"تم إرسال إشارة {signal} لـ {symbol} - قوة: {strength}/10")
                        await self.telegram_notifier.send_signal_alert(signal_data)
                        return signal_data
                    else:
                        safe_log_error(f"فشل إرسال الإشارة لـ {symbol}")
                else:
                    safe_log_info(f"إشارة مكتشفة ولكن الإرسال معطل: {signal} لـ {symbol} - قوة: {strength}/10")
                    await self.telegram_notifier.send_signal_alert(signal_data)
                    return signal_data
            else:
                safe_log_info(f"لا توجد إشارات قوية لـ {symbol} - الإشارة: {signal} - القوة: {strength}/10")
                return None
...
async def periodic_scanner_task():
    ...
    safe_log_info("بدء المهمة الدورية للمسح الضوئي للإشارات")
...
async def startup_event():
    """حدث بدء التشغيل"""
    safe_log_info("بدء تشغيل ماسح الإشارات المتقدم")
...
async def shutdown_event():
    """حدث إيقاف التشغيل"""
    safe_log_info("إيقاف ماسح الإشارات المتقدم")
