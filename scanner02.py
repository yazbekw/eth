...
system_stats = {
    ...
    "data_source_stats": {
        ...
        "alternative": {"success": 0, "failed": 0}
    },
...
class MultiSourceDataFetcher:
    """مصادر متعددة لجلب البيانات مع fallback تلقائي"""
    
    def __init__(self):
        self.client = httpx.AsyncClient(timeout=30.0)
        self.last_request_time = 0
        self.min_request_interval = 1.0  # ثانية واحدة بين الطلبات
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data and len(data) > 0:
                    ...
                    logger.warning(f"⚠️ لا توجد بيانات من Binance لـ {symbol}")
    ...
    async def get_alternative_data(self, symbol: str, interval: str, limit: int = 50) -> Optional[Dict]:
        """مصدر بديل للبيانات (CoinGecko)"""
        try:
            ...
            if not coin_id:
                logger.warning(f"⚠️ لا يوجد رمز بديل لـ {symbol}")
                return None
            
            # تحويل الإطار الزمني
            interval_mapping = {
                '1h': '1',  # 1 يوم بدلاً من 1 ساعة (لتبسيط API)
            }
            
            days = interval_mapping.get(interval, '1')
            ...
            if response.status_code == 200:
                ...
                if 'prices' in data and len(data['prices']) > 0:
                    system_stats["data_source_stats"]["alternative"]["success"] += 1
                    
                    # تحويل البيانات إلى تنسيق مشابه لـ Binance
                    prices_data = []
                    for i, price_point in enumerate(data['prices']):
                        # إنشاء بيانات مشابهة لـ Binance Klines
                        ...
                        prices_data.append([
                            ...
                            "1",        # number of trades
                            "1000",     # taker buy base asset volume
                            "1000",     # taker buy quote asset volume
                            "0"         # ignore
                        ])
                    
                    logger.info(f"✅ تم جلب {len(prices_data)} نقطة سعرية من المصدر البديل لـ {symbol}")
                    return prices_data[:limit]  # تقليل العدد إذا كان كبيراً
                
            system_stats["data_source_stats"]["alternative"]["failed"] += 1
            logger.warning(f"⚠️ فشل جلب البيانات من المصدر البديل لـ {symbol}")
            return None
            
        except Exception as e:
            system_stats["data_source_stats"]["alternative"]["failed"] += 1
            logger.error(f"❌ خطأ في جلب البيانات من المصدر البديل لـ {symbol}: {e}")
            return None
    
    async def get_coin_data(self, symbol: str, interval: str) -> Dict[str, Any]:
        """جلب البيانات من مصادر متعددة مع fallback تلقائي"""
        logger.info(f"🔍 محاولة جلب بيانات {symbol}...")
        
        # المحاولة الأولى: Binance
        data = await self.get_binance_data(symbol, interval)
        
        # المحاولة الثانية: المصدر البديل إذا فشلت Binance
        if data is None:
            logger.info(f"🔄 المحاولة مع المصدر البديل لـ {symbol}...")
            data = await self.get_alternative_data(symbol, interval)
        ...
        try:
            ...
            source = "binance" if len(data) > 10 else "alternative"  # تقدير المصدر
            
            logger.info(f"✅ تم جلب {len(prices)} سعر من {source} لـ {symbol} - آخر سعر: {prices[-1]:.2f}")
            ...
            logger.error(f"❌ خطأ في معالجة البيانات لـ {symbol}: {e}")
            return {
                "signal": "none",
                "confidence": 0,
                "reasons": [f"خطأ في معالجة البيانات: {str(e)}"],
                "data_source": "error"
            }
...
class AdvancedSignalEngine:
    ...
    async def analyze_coin(self, coin_symbol: str, binance_symbol: str) -> Dict[str, Any]:
        ...
        # تطبيق الاستراتيجيات على البيانات
...
class TelegramNotifier:
    ...
    def _build_advanced_signal_message(self, coin: str, signal_data: Dict[str, Any]) -> str:
        ...
        source_emoji = "🔵" if data_source == "binance" else "🟡"
    ...
    async def send_heartbeat(self, executor_connected: bool, signals_count: int = 0, 
        ...
        try:
            ...
            # إحصائيات الاستراتيجيات ومصادر البيانات
                ...
                for coin, analysis in recent_analysis.items():
                    if analysis and analysis.get('strategies_analysis'):
                        ...
                        if buy_signals and sell_signals:
                            ...
                            message += f"⚡ **{coin.upper()}:** تضارب (🟢 شراء: {buy_avg:.1f}% | 🔴 بيع: {sell_avg:.1f}%) {source_emoji}\n"
                            ...
                            message += f"{emoji} **{coin.upper()}:** اتجاه شراء ({len(buy_signals)}/3) - قوة: {avg_confidence:.1f}% {source_emoji}\n"
                            ...
                            emoji = "🔴" if avg_confidence >= 40 else "🟠"
                            message += f"{emoji} **{coin.upper()}:** اتجاه بيع ({len(sell_signals)}/3) - قوة: {avg_confidence:.1f}% {source_emoji}\n"
                            ...
                            if all_confidences:
                                ...
                                if max_confidence > 30:
                                    message += f"⚪ **{coin.upper()}:** إشارات ضعيفة (أعلى: {max_confidence}%) {source_emoji}\n"
                                else:
                                    message += f"⚫ **{coin.upper()}:** لا توجد إشارات قوية (أعلى: {max_confidence}%) {source_emoji}\n"
                            else:
                                message += f"⚫ **{coin.upper()}:** لا توجد إشارات {source_emoji}\n"
                    else:
                ...
                try:
                    ...
                    for strat_name, strat_data in strategies_analysis.items():
                        if strat_data.get('signal') != 'none' and strat_data.get('confidence', 0) > 0:
                            active_strategies.append(f"{strat_name}: {strat_data['signal']}({strat_data['confidence']}%)")
                    
                    data_source = analysis_result.get('data_source', 'unknown')
                    source_emoji = "🔵" if data_source == "binance" else "🟡"
                    
                    if active_strategies:
                        logger.info(f"📈 {coin_key} - إشارات نشطة: {', '.join(active_strategies)} {source_emoji}")
                    else:
...
async def send_unified_alert(scan_results: List[Dict]):
    ...
    try:
        ...
        for i, result in enumerate(scan_results, 1):
            ...
            message += f"{emoji} **{result['coin'].upper()}:** {signal_type} ({confidence}%) - {winning_strategies}/3 استراتيجيات {source_emoji}\n"
...
async def root():
    return {
        ...
        "data_sources": ["binance", "alternative"],
...
async def startup_event():
    ...
    logger.info(f"📡 مصادر البيانات: Binance + CoinGecko (بديل)")
