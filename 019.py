...
class AdvancedCryptoBot:
            ...
            try:
                ...
                if reason:
                    position.update({
                        ...
                        'exit_rsi': float(row['rsi']),
                        'exit_macd': float(row['macd']),
                        'price_change': float((current_price - position['entry_price']) / position['entry_price'] * 100)
                    })
    ...
    def analyze_loss_reasons(self):
        """تحليل أسباب الخسارة وإرجاع اقتراحات التحسين"""
        if not self.trades:
        ...
        if stop_loss_trades:
            ...
            if len(stop_loss_trades) > len(losing_trades) * 0.7:
                analysis += "• مراجعة مستويات وقف الخسارة (قد تكون ضيقة جداً)\n"
        
        # تحليل صفقات الخروج الزمني
        if time_exit_trades:
            analysis += f"\n⏰ صفقات الخروج الزمني ({len(time_exit_trades)} صفقة):\n"
            
            avg_duration = np.mean([t['duration_hours'] for t in time_exit_trades])
            analysis += f"• متوسط المدة: {avg_duration:.1f} ساعة\n"
            
            # تحليل تغير السعر
            avg_price_change = np.mean([t['price_change'] for t in time_exit_trades])
    ...
    def generate_improvement_suggestions(self):
        """توليد اقتراحات تحسين عامة بناءً على الأداء"""
        if not self.trades:
...
- نطاق قوة الإشارة: {min_strength}-{max_strength}
    ...
    def send_telegram_report(self):
        """إرسال التقرير عبر التلغرام مع تحليل مفصل"""
        try:
            ...
            main_report = self.generate_report()
            bot.send_message(chat_id, main_report)
            print("✅ تم إرسال التقرير الرئيسي إلى التلغرام")
            
            # إرسال تحليل أسباب الخسارة
            loss_analysis = self.analyze_loss_reasons()
            if len(loss_analysis) > 4000:
                parts = [loss_analysis[i:i+4000] for i in range(0, len(loss_analysis), 4000)]
                for i, part in enumerate(parts):
                    bot.send_message(chat_id, f"تحليل الخسائر - الجزء {i+1}:\n{part}")
            else:
                bot.send_message(chat_id, loss_analysis)
            print("✅ تم إرسال تحليل الخسائر إلى التلغرام")
            
            # إرسال اقتراحات التحسين
            improvements = self.generate_improvement_suggestions()
            if len(improvements) > 4000:
                parts = [improvements[i:i+4000] for i in range(0, len(improvements), 4000)]
                for i, part in enumerate(parts):
                    bot.send_message(chat_id, f"اقتراحات التحسين - الجزء {i+1}:\n{part}")
            else:
                bot.send_message(chat_id, improvements)
            print("✅ تم إرسال اقتراحات التحسين إلى التلغرام")
...
def main():
    print("🚀 بدء تشغيل البوت المتقدم مع تحليل مفصل للخسائر...")
    print("=" * 60)
