...
TRADE_CONFIG = {
    ...
    'peak_hours': [0, 4, 8, 12, 16, 20],  # ساعات UTC
...
class AdvancedCryptoBot:
    ...
    def fetch_binance_data(self, days=30):
        ...
        try:
            ...
            required_candles = int(days * 24 / self._interval_to_hours(interval)) + 100
            ...
            if ENABLE_LOGGING:
                logger.info(f"تم جلب {len(self.data)} شمعة")
    ...
    def generate_sample_data(self, days):
        ...
        interval_hours = self._interval_to_hours(self.trade_config['timeframe'])
        dates = pd.date_range(start=start_date, end=end_date, freq=f'{int(interval_hours*60)}T')
    ...
    def open_position(self, direction, strength, row, details):
        try:
            ...
            if ENABLE_DYNAMIC_POSITION_SIZING:
                size_factor = strength / 10
            else:
                size_factor = 1.0
            position_value = self.current_balance * base_size * size_factor * self.trade_config['leverage']

            entry = row['close']
            atr = row['atr'] if ENABLE_ATR_SL_TP and 'atr' in row else entry * 0.01
    ...
    def generate_report(self):
        ...
        balance_series = self.initial_balance
        balance_history = [balance_series]
        for t in self.trades:
            balance_series += t['size'] * t['pnl']
            balance_history.append(balance_series)
